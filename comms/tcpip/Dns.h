...
protected:
	uint16_t BuildRequest(const char* hostname)
	{
		...
		udp->write((uint8_t*)&_id, sizeof(_id));
		...
		while (*end)
		{
			...
			if (end-start > 0)
			{
				...
				udp->write((uint8_t*)start, end-start);
			}
			start = end+1;
		}
	...
	uint16_t ProcessResponse(uint16_t aTimeout, IPv4Address& ip)
	{
		...
		if (udp->available() < DNS_HEADER_SIZE)
			return TRUNCATED;
		udp->read(header, DNS_HEADER_SIZE);
		...
		for (uint16_t i =0; i < answerCount; i++)
		{
			...
			udp->read((uint8_t*)&answerType, sizeof(answerType));
			udp->read((uint8_t*)&answerClass, sizeof(answerClass));
			...
			udp->read((uint8_t*)&header_flags, sizeof(header_flags));
